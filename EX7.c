...
int main() {
  ...
  DDRB = 0b00000011; //PB1 como saída (transistor do display)
  ...
  PCICR = 0b00000011; //hab. interrupção por mudança de sinal no PORTB e PORTC
  PCMSK0 = 0b00000100; // hab. o pino PCINT2 para gerar interrupção
...
ISR(PCINT0_vect) {
  // Altera qual display irá exibir o número, acionando o transistor correspondente.
  if (!(PINB & (1 << 2))) PORTB = 1;
  else PORTB = (1 << 1);
